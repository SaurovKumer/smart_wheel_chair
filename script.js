...
function onResults(results) {
    ...
    if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
        ...
        if (fingersUpCount === 4) { sendCommand("F"); } 
        else if (fingersUpCount === 0) { sendCommand("S"); } 
        else if (fingersUpCount === 2 && isIndexUp && isMiddleUp) { sendCommand("R"); } 
        else if (fingersUpCount === 1 && isIndexUp) { sendCommand("L"); } 
        else if (fingersUpCount === 3) { sendCommand("B"); }
    }
    canvasCtx.restore();
}
